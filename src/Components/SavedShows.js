...
const SavedShows = () => {
	...
	useEffect(() => {
		onSnapshot(doc(db, "users", `${user?.email}`), (doc) => {                      //help
	 		setMovies(doc.data()?.savedShows)
		})
	}, [user?.email])

	const movieRef = doc(db, "users", `${user?.email}`);               //all help, also sign up error dont work
	...
	return (
		<>
		<h2 className="text-gray-400 font-bold md:text-xl p-4 pl-7 text-transform: uppercase">My Shows</h2>
